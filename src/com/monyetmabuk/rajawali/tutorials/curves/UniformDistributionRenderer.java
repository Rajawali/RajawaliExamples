...
/**
 * This example shows you how you can reparametrize a curve in order for
 * its points to be uniformly distributed. This can be useful when using
 * curves with animation. The animation speeds will be more or less constant.
 * This will alter the shape of the curve depending on the resolution parameter.
 * The higher the resolution, the more closely it will resemble the original curve.
 * 
 * @author dennis.ippel
 * 
 */
public class UniformDistributionRenderer extends RajawaliRenderer {
	...
	private void drawCurve(ICurve3D curve, int color, Vector3 position) {
		...
		for (int i = 0; i <= NUM_POINTS; i++) {
			Vector3 pos = curve.calculatePoint((float) i / (float) NUM_POINTS);
			points.add(pos);
		}
