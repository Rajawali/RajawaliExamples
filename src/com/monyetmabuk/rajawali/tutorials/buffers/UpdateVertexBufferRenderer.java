...
public class UpdateVertexBufferRenderer extends RajawaliRenderer {
	/**
	...
	/**
	 * The interpolated normalized time. This will be used to get the {@link Vector3}s from the curve
	 */
	private float mInterpolation;
	/**
	 * The start time will be used to calculate the delta time. This will allow us to have frame rate
	 * independent constant animation speed.
	 */
	...
	/**
	 * The vertex buffer that will store the vertices that we'll generate based on the spline and some randomness
	 */
	...
	/**
	 * Stores a vertex that was used by the previous triangle. This way we can connect all vertices
	 * and create a ribbon.
	 */
	...
	public void initScene()
	{
		...
		for (int i = 0; i < 128; i++) {
			mCurve.addPoint(new Vector3(-rh + (Math.random() * r), -rh + (Math.random() * r), -rh + (Math.random() * r)));
		}
		...
		mVertexBuffer = ByteBuffer.allocateDirect(9 * Geometry3D.FLOAT_SIZE_BYTES)
				.order(ByteOrder.nativeOrder()).asFloatBuffer();
		// -- Indicate that we are going to update the vertex buffer very frequently
		mCurveTris.getGeometry().changeBufferUsage(mVertexBufferInfo, GLES20.GL_DYNAMIC_DRAW);
		...
		RotateAroundAnimation3D anim = new RotateAroundAnimation3D(new Vector3(), Axis.Y, 10);
	...
	public void onDrawFrame(GL10 glUnused) {
		...
		mCurveTris.getGeometry().changeBufferData(mVertexBufferInfo, mVertexBuffer, mCurrentVertexIndex * 3,  9);
